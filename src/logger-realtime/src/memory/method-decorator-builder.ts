...
export function methodDecoratorBuilder(
    ...
    return function methodDecorator(name: string, logBodyOnChange?: Partial<LoggerBodyType>, update?: boolean) {
        ...
        //todo перенести всі операції з з базою даних (створення ентітів шукати апдейтити додавати) з сервісів в логер щоб задиплоїти як окремий репозиторій
        //todo  тут ts а не js, перевірити доробити і перевірити на коремому проекті чи орм скачуватиметься
        //todo доробити перетворення  з ts в js і завантаження https://www.youtube.com/watch?v=xnfdm-s8adI
